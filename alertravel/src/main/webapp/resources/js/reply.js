	...
	return `
		...
		data-username="${reply.username}">
			<div class="reply-title my-2 d-flex justify-content-between">
				...
				<div class="btn-group">
					${username && (username == reply.username) ? replyUpdatable : ''} 
				</div>
			</div>
...
// 답글 추가
async function addReply(el, username) { //el이 답글 작성하고 누르는 "확인"버튼
	...
	let reply = { cno, username, content };

	// REPLY POST API 호출
	reply = await rest_create(REPLY_URL, reply);
	console.log(reply);

	const replyEl = $(createReplyTemplate(reply, username));
